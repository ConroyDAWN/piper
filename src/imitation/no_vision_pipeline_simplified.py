...
- action 指令（Δx,Δy,Δz 或 Δx,Δy,Δz,Δrvec_x,Δrvec_y,Δrvec_z）
"""
...
def _vee(mat: np.ndarray) -> np.ndarray:
    return np.array(
        [
            mat[2, 1] - mat[1, 2],
            mat[0, 2] - mat[2, 0],
...
def euler_xyz_to_rotmat(rx: float, ry: float, rz: float) -> np.ndarray:
    """
    欧拉角 [rx, ry, rz] -> 旋转矩阵，采用 R = Rz(rz) @ Ry(ry) @ Rx(rx)。
    """
...
def rotmat_to_euler_xyz(rot: np.ndarray) -> np.ndarray:
    """
    旋转矩阵 -> 欧拉角 [rx, ry, rz]，与 euler_xyz_to_rotmat 保持同一约定。
    """
    r20 = float(rot[2, 0])
    if abs(r20) < 1.0 - SO3_EPS:
        ry = -np.arcsin(r20)
        cy = np.cos(ry)
        rx = np.arctan2(rot[2, 1] / cy, rot[2, 2] / cy)
        rz = np.arctan2(rot[1, 0] / cy, rot[0, 0] / cy)
    else:
        # gimbal lock：固定 rz=0，退化求 rx
...
def so3_exp(rotvec: np.ndarray) -> np.ndarray:
    """
    so(3) 向量（旋转向量） -> SO(3) 旋转矩阵。
    """
    w = np.asarray(rotvec, dtype=np.float64).reshape(3)
    theta = np.linalg.norm(w)
    if theta < SO3_EPS:
        w_hat = _skew(w)
        return np.eye(3, dtype=np.float64) + w_hat

    w_hat = _skew(w)
    theta2 = theta * theta
    a = np.sin(theta) / theta
    b = (1.0 - np.cos(theta)) / theta2
    return np.eye(3, dtype=np.float64) + a * w_hat + b * (w_hat @ w_hat)


def so3_log(rot: np.ndarray) -> np.ndarray:
    """
    SO(3) 旋转矩阵 -> so(3) 向量（旋转向量）。
    """
    ...
    if theta < SO3_EPS:
        return _vee(rot - rot.T)

    sin_theta = np.sin(theta)
    if abs(sin_theta) < SO3_EPS:
        diag = np.diag(rot)
        axis = np.array(
            [
                np.sqrt(max((diag[0] + 1.0) * 0.5, 0.0)),
                np.sqrt(max((diag[1] + 1.0) * 0.5, 0.0)),
                np.sqrt(max((diag[2] + 1.0) * 0.5, 0.0)),
            ],
            dtype=np.float64,
        )
        # 依据反对称项确定符号
        ...
        if norm_axis < SO3_EPS:
            return np.zeros(3, dtype=np.float64)
        return axis / norm_axis * theta

    return (theta / (2.0 * sin_theta)) * np.array(
        [
            rot[2, 1] - rot[1, 2],
            rot[0, 2] - rot[2, 0],
            rot[1, 0] - rot[0, 1],
...
def action_from_obs(prev_obs: Dict, curr_obs: Dict) -> List[float]:
    ...
    由相邻两帧 obs 构造 6D action，并归一化到 [-1, 1]。
    ...
    r_delta = r0.T @ r1
    delta_rotvec = so3_log(r_delta).astype(np.float32)
...
def add_file_metadata(f: h5py.File, args: argparse.Namespace, total_samples: int,freq) -> None:
    env_args = {
        ...
        "action_definition": "delta_tcp_xyz_plus_delta_rotvec_normalized",
        ...
        "rotation_delta_repr": "so3_rotvec",
        "q_units": "rad",
        "dq_units": "rad/s",
    }
...
def _compose_next_pose(curr_pose: np.ndarray, delta_action: np.ndarray) -> np.ndarray:
    ...
    把 delta action 叠加到当前 tcp_pose，得到下一步目标 pose。
    - 平移部分: 直接加和
    - 旋转部分: 使用 SO(3) 乘法 R_next = R_curr @ Exp(delta_rotvec)
    ...
    if delta_action.shape[0] == 3:
        ...
        next_pose[:3] += delta_action[:3]
        r_curr = euler_xyz_to_rotmat(float(curr_pose[3]), float(curr_pose[4]), float(curr_pose[5]))
        r_delta = so3_exp(np.asarray(delta_action[3:6], dtype=np.float64))
        r_next = r_curr @ r_delta
        next_pose[3:6] = rotmat_to_euler_xyz(r_next)
...
def _send_joint_command(
    ...
    if joints.shape[0] != 6:
        raise ValueError(f"joints must be 6D, got shape {joints.shape}")
...
def cmd_replay(args: argparse.Namespace) -> None:
    ...
    with h5py.File(in_path, "r") as f:
        ...
        try:
            ...
            #     first_joints = np.asarray(obs_q[0], dtype=np.float64).copy()
            #     print(f"[INFO] move to first trajectory joints: {first_joints.tolist()}")
            ...
            #         motion_timeout=15.0,
            #     )
            #     print(f"[INFO] move_j to first trajectory joints done, ok={joint_ok}")

            #     if not joint_ok:
            #         raise RuntimeError("move_j 到轨迹起点失败")

            #     input("[INFO] 已到达轨迹起点，按回车开始正式回放")
            # else:
            #     print("[WARN] HDF5 中缺少 obs/q，跳过轨迹起点对齐")
