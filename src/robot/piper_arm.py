...
class PiperArm:
    ...
    def wait_motion_done(
        ...
        verbose: bool = True,
        expected_pose: Optional[Pose6] = None,
        expected_joints: Optional[Joint6] = None,
        position_tolerance: float = 0.005,
        angle_tolerance: float = 0.05,
    ) -> bool:
        """
        等待机械臂运动完成。
        默认以 robot.get_arm_status().msg.motion_status == 0 为完成信号。
        若提供 expected_pose / expected_joints，则会额外检查当前位置是否已接近目标，
        避免底层状态位提前返回 0 但机械臂实际上未到位的误判。
        """
        ...
        if expected_pose is not None:
            self._check_len(expected_pose, 6, "expected_pose")
        if expected_joints is not None:
            self._check_len(expected_joints, 6, "expected_joints")
        ...
        while True:
            ...
            if motion_done and expected_pose is not None:
                try:
                    curr_pose = self.get_tcp_pose6()
                except Exception:
                    curr_pose = None
                if curr_pose is None or not self._vec6_close(
                    curr_pose,
                    expected_pose,
            ...
            if motion_done and expected_joints is not None:
                try:
                    curr_joints = self.get_joint_position_rad()
                except Exception:
                    curr_joints = None
                if curr_joints is None or not self._vec6_close(
                    curr_joints,
                    expected_joints,
    ...
    def set_motion_mode_l(self) -> Any:
        """设置为直线运动模式 L。"""
        return self.robot.set_motion_mode(self.robot.OPTIONS.MOTION_MODE.L)
    ...
    def move_j(
        ...
        return (
            self.wait_motion_done(timeout=timeout, expected_joints=joints)
            if wait
            else True
        )
    ...
    def move_l(
        ...
        return (
            self.wait_motion_done(timeout=timeout, expected_pose=pose)
            if wait
            else True
        )
    ...
    def move_c(
        ...
        return (
            self.wait_motion_done(timeout=timeout, expected_pose=end_pose)
            if wait
            else True
        )
    ...
    def go_home(
        ...
        """回零位。"""
        home_joints = [0.0] * 6
        return self.move_j(home_joints, wait=wait, timeout=timeout)
    ...
    def _vec6_close(
        ...
        for idx in range(6):
            tolerance = position_tolerance if idx < 3 else angle_tolerance
            if abs(float(lhs[idx]) - float(rhs[idx])) > tolerance:
                return False
        return True
