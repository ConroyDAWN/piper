...
def wait_feedback_ready(robot, timeout: float = 3.0, poll_interval: float = 0.05) -> bool:
    """Wait until at least one status frame is available."""
    start_t = time.monotonic()
    while True:
        if robot.get_arm_status() is not None:
            return True
...
def _safe_get_arm_status_msg(robot):
    status = robot.get_arm_status()
    return status.msg if status is not None else None


def enable_with_timeout(robot, timeout: float = 12.0, retry_interval: float = 0.01) -> bool:
    """Retry enable until success or timeout. Try single-joint fallback for robustness."""
    start_t = time.monotonic()

    # First try: enable all joints directly.
    ...
    # Fallback: try enabling each joint separately.
    while time.monotonic() - start_t <= timeout:
        ok_all = True
        for joint_index in range(1, 7):
            joint_ok = robot.enable(joint_index)
            ok_all = ok_all and joint_ok
            time.sleep(retry_interval)
        if ok_all and all(robot.get_joints_enable_status_list()):
            return True
        time.sleep(retry_interval)
...
def print_enable_diagnostics(robot) -> None:
    print("[DIAG] joints_enable:", robot.get_joints_enable_status_list())
    print("[DIAG] arm_status:", _safe_get_arm_status_msg(robot))
    for i in range(1, 7):
        ds = robot.get_driver_states(i)
        if ds is None:
            print(f"[DIAG] driver_state_{i}: None")
        else:
            print(f"[DIAG] driver_state_{i}.foc_status: {ds.msg.foc_status}")
...
print("[INFO] enable")
if not wait_feedback_ready(robot, timeout=3.0):
    print("[WARN] no status feedback in 3s, CAN data may be missing")

if not enable_with_timeout(robot, timeout=8.0):
    print_enable_diagnostics(robot)
    raise RuntimeError("enable 失败：超时")

print("[INFO] enabled joints:", robot.get_joints_enable_status_list())
robot.move_j([0.0] * 6)
time.sleep(0.01)
robot.disable()
